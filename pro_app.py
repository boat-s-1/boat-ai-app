...
with tab5:
    ...
    # 無料版：直近30走
    place_df = (
        place_df.sort_values("日付", ascending=False)
                .groupby("艇番", as_index=False)
                .head(30)
    )
    ...
    st.caption(f"会場：{race_place}（直近30走平均との差補正）")
    ...
    for i in range(1, 7):
        ...
        with input_cols[i - 1]:
            ...
            tenji_input[i] = st.number_input(
                "展示", step=0.01, format="%.2f",
                key=f"mix_tenji_{i}"
            )

            isshu_input[i] = st.number_input(
                "一周", step=0.01, format="%.2f",
                key=f"mix_isshu_{i}"
            )

            st_input[i] = st.number_input(
                "ST", step=0.01, format="%.2f",
                key=f"mix_st_{i}"
            )
    ...
    for _, r in result_df.iterrows():
        ...
        if img_base64 == "":
            ...
            img_html = f'<img src="data:image/png;base64,{img_base64}" height="48">'

        html = f"""
        <div style="margin-bottom:10px;">
            <div style="margin-left:{offset}px; display:flex; align-items:center;">
                {img_html}
