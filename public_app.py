...
def show_main_page():
    ...
    with tab1:
        ...
        # --- TAB2: 使い方 ---
    with tab2:
        st.header("🔰 競艇予想Pro 完全攻略ガイド")
        
        # --- セクション1: 事前簡易予想 ---
        st.markdown("### 🎯 STEP1：事前簡易予想（期待値の把握）")
        st.info("""
        展示航走が出る前に、モーター、当地勝率、枠番勝率、スタート(ST)の4項目から
        **『そのレースにおける各艇の地力』**をパーセンテージで算出します。
        """)

        with st.expander("👉 事前簡易予想の使いかた・見かた", expanded=True):
            st.markdown("""
            **1. 評価を入力する**
            出走表を参考に、各艇の「モーター」「当地」「枠番」「ST」を ◎〜× で選択します。
            
            **2. 予想％（パーセンテージ）に注目**
            計算ボタンを押すと、全6艇の合計が100%になるように期待値が配分されます。
            * **25%以上**: そのレースの絶対的な軸候補です。
            * **20%前後**: 対抗、あるいは逆転の可能性がある実力艇です。
            * **10%以下**: 展開の助けが必要な評価となります。

            **💡 プロの狙い目:**
            1位と2位の％差が大きく、1位が1号艇の場合は**『鉄板本命レース』**。
            逆に％が横並び（18%〜20%が複数）の場合は、**『大混戦の穴レース』**と判断できます。
            """)

        st.divider()

        # --- セクション2: 統計解析（先ほどの説明をここに続ける） ---
        st.markdown("### 📊 STEP2：会場別統計解析（直前情報の補正）")
        st.write("直前展示が発表されたら、数値を入力して『真の足の良さ』を導き出します。")
        # （ここへ先ほどの統計解析の解説を繋げる）
