...
def show_main_page():
    ...
    with tab2:
        ...
        # --- 2. 精度検証アピール（データがある場合のみ数値を表示） ---
        st.markdown("### 📈 嘘偽りのない「ロジックの精度」を公開中")
        
        # 変数が存在するかチェック（存在しない場合は初期値を設定）
        ...
        with st.expander("🚀 STEP3：スタート指数（スリット攻防）", expanded=False):
            ...
            * **目視評価の活用**:
                - **◎**: スリットから明らかに出ている
                - **×**: 届いていない、または行き足に不安
            * **スタート指数**: 数値が高いほど、1マークで先手を取れる確率がアップ。
            """)
        ...
        with st.expander("🌊 STEP4：条件補正（水面状況の分析）", expanded=False):
            ...
            * **主な設定項目**: 会場、風向き、風速、波高をスライダーで調整。
            * **条件平均との差**: 全体平均からどれほどズレているかを算出。**数値がマイナスに大きいほど、その条件において有利な艇番**を示しています。
            """)
