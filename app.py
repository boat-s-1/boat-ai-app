...
with st.sidebar.expander("⏱ 左：タイム解析データ", expanded=True):
    ...
    st.write("各艇の【生展示タイム】")
    t_cols = st.columns(3)
    raw_t = [t_cols[i % 3].number_input(f"{i+1}号", value=6.50+(i*0.01), step=0.01, format="%.2f", key=f"raw_{i}") for i in range(6)]
    st.write("独自補正（イン/場など）")
    o_cols = st.columns(3)
    offs = [o_cols[i % 3].number_input(f"{i+1}補正", value=(-0.03 if i==0 else 0.0), step=0.01, format="%.2f", key=f"off_{i}") for i in range(6)]
...
<style>
    ...
    .box-heading { font-weight: bold; border-bottom: 2px dashed #333; margin-bottom: 12px; font-size: 18px; display: flex; align-items: center; gap: 8px; }
    ...
    .talk-line { display: flex; gap: 10px; background: #fefce8; padding: 10px; border-radius: 10px; border: 1px solid #fef08a; font-family: 'Yomogi', cursive; font-size: 14px; margin-bottom: 8px; }
...
<div class="paper-container">
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 4px double #333; padding-bottom: 15px; margin-bottom: 25px;">
        ...
        <div style="background: linear-gradient(to right, #ffecd2, #fcb69f); padding: 8px 60px; border: 2px solid #333; border-radius: 50px; font-size: 40px; font-weight: bold; color: #1a1a1a;">Birthstones新聞</div>
    ...
    <div style="display: flex; gap: 20px; align-items: stretch;">
        <div style="flex: 1.7; border: 2px solid #333; border-radius: 12px; padding: 15px; background: #f8fafc;">
            <div class="box-heading">📊 公式：展示タイム生データ</div>
            <table class="data-table">
                <tr><th>1号</th><th>2号</th><th>3号</th><th>4号</th><th>5号</th><th>6号</th></tr>
                <tr>{raw_tds}</tr>
            </table>
            <div style="margin-top: 40px;" class="box-heading">🔍 独自：補正展示タイム解析</div>
            <div style="background: white; border: 2px solid #333; border-radius: 8px; padding: 20px;">
                <table class="data-table">
                    <tr><th>1号</th><th>2号</th><th>3号</th><th>4号</th><th>5号</th><th>6号</th></tr>
                    <tr>{final_tds}</tr>
                </table>
                <div style="margin-top: 15px; display: flex; gap: 20px; font-size: 12px; font-weight: bold;">
        ...
        <div style="flex: 1.3; border: 2px solid #333; border-radius: 12px; padding: 15px; background: #fffdf5;">
            <div class="box-heading" style="color: #854d0e;">💬 3人の放課後トーク</div>
            ...
            <div class="talk-line"><div class="face-badge">一果</div>「{i_msg}」</div>
            <div style="background: #1e293b; color: white; padding: 4px; text-align: center; border-radius: 4px; font-size: 13px; font-weight: bold; margin: 15px 0 8px 0;">初音格付け：{h_eval}</div>
            <div class="talk-line" style="background:#f0f9ff; border-color:#3b82f6;"><div class="face-badge">初音</div>「{h_msg}」</div>
            ...
            <div class="talk-line" style="background:white; border-color:#eab308;"><div class="face-badge">キイナ</div>「{k_msg}」</div>
        </div>
    </div>
...
st.markdown(full_newspaper_html, unsafe_allow_html=True)    }

    /* 各セクションの余白を詰める */
    .news-grid { display: flex !important; gap: 20px !important; }
    .news-box {
        border: 2px solid #333 !important; border-radius: 10px;
        padding: 15px; background: #fff; margin-bottom: 20px;
    }
    .box-heading {
        font-weight: bold; border-bottom: 2px dashed #333;
        margin-bottom: 12px; font-size: 18px; display: flex; align-items: center; gap: 8px;
    }
    ...
    .talk-line {
        ...
        border-radius: 10px; border: 1px solid #fef08a;
        font-family: 'Yomogi', cursive; font-size: 14px; margin-bottom: 8px;
    }
    .face-badge {
        width: 40px; height: 40px; border-radius: 50%; border: 1.5px solid #333;
        background: #fff; display: flex; align-items: center; justify-content: center;
        font-size: 10px; flex-shrink: 0; font-weight: bold; font-family: 'Kosugi Maru';
    }
</style>
""", unsafe_allow_html=True)

# --- 4. HTMLを一括で組み立て（これが「しょぼさ」を消す鍵です） ---
needle_angle = (i_exp - 50) * 1.8
raw_tds = "".join([f"<td>{t:.2f}</td>" for t in raw_t])
final_tds = "".join([f'<td style="{get_t_color(t)}">{t:.2f}</td>' for t in final_t])

# 全体を一つの巨大な文字列として構築することで、隙間を完全に排除します
full_newspaper_html = f"""
<div class="paper-container">
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 4px double #333; padding-bottom: 15px; margin-bottom: 25px;">
        <div style="font-size: 12px; line-height: 1.4;">3年1組<br>BOAT STRIKE! 係</div>
        <div style="background: linear-gradient(to right, #ffecd2, #fcb69f); padding: 8px 60px; border: 2px solid #333; border-radius: 50px; font-size: 40px; font-weight: bold; color: #1a1a1a;">Birthstones新聞</div>
        <div style="text-align: right; font-size: 12px;">{formatted_date} 第31号<br>🧭 きずな班 編集</div>
    </div>
    ...
    <div class="news-grid">
        <div style="flex: 1.7;">
            <div class="news-box" style="background: #f8fafc; height: 100%; margin-bottom: 0;">
                ...
                <p style="font-size: 11px; color: #64748b; margin-top: 25px; font-style: italic;">※場特性を考慮した実戦値。1位:赤、2位:黄で表示。</p>
            </div>
        </div>

        <div style="flex: 1.3;">
            <div class="news-box" style="background: #fffdf5; border-color: #f59e0b; height: 100%; margin-bottom: 0;">
                <div class="box-heading" style="color: #854d0e; border-color: #f59e0b;">💬 3人の放課後トーク</div>
