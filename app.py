...
with st.sidebar.expander("一果の「特集」", expanded=True):
    ...
    i_text = st.text_area("メッセージ", value="平均より高いよ！壁役も安定してるし、ここは一果にお任せ！")

with st.sidebar.expander("初音の「データ」", expanded=True):
    h_bet = st.text_input("推奨買い目", value="1-4-全")
    h_data = st.text_area("データ（改行で箇条書き）", value="・補正タイム1位\n・当地勝率No.1")

with st.sidebar.expander("キイナの「穴」", expanded=True):
    k_eval = st.selectbox("穴判定", ["買わなきゃ損！", "GO", "見（ケン）"])
    k_text = st.text_area("メッセージ ", value="4が凹めば5のまくり差し炸裂！買わなきゃ損だよ！")

# --- 2. CSSスタイル定義 ---
# レンダリングバグを防ぐため、CSSは独立させて先に読み込ませます
st.markdown("""
<style>
...
# 各変数を安全に埋め込んだHTMLを作成
# 「f-string」を使いつつ、タグが漏れないようにシンプルに構成します
html_content = (
    '<div class="paper-frame">'
    '    <div class="news-header">'
    '        <div style="font-size: 24px;">✨💎✨</div>'
    f'        <div class="main-banner">Birthstones新聞</div>'
    f'        <div style="text-align: right; font-size: 12px; color: #4a5568;">'
    f'            {formatted_date} 第15号<br>🧭 3年1組 きずな班'
    '        </div>'
    '    </div>'
    '    <div class="news-grid">'
    '        <div class="tokushu-side">'
    '            <div class="info-box" style="border-color: #f6ad55; background: #fffaf0;">'
    f'                <div class="info-title">🚩 特集：一果のイン逃げ判定</div>'
    f'                <p style="font-size: 13px; margin-bottom: 15px;">場所：ボートレース{i_place} ({i_race}) / 天気：晴れ☀️</p>'
    ...
    f'                        <span style="font-size: 30px; font-weight: bold; color: #e53e3e;">{diff_str}</span>'
    '                    </div>'
    '                    <div style="text-align: center;">'
    f'                        <div style="font-size: 14px; margin-bottom: 5px;">期待値：{i_exp}%</div>'
    ...
    f'                <div style="background: #2d3748; color: white; padding: 5px; text-align: center; border-radius: 5px; margin-bottom: 10px; font-weight: bold;">{h_bet}</div>'
    ...
    f'                <p style="text-align: center; font-weight: bold; color: #d69e2e; margin-bottom: 10px;">判定：{k_eval}</p>'
