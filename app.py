...
with st.sidebar.expander("⏱ 左エリア：展示タイム解析", expanded=True):
    ...
    st.write("【生展示タイム】")
    t_cols = st.columns(3)
    raw_t = [t_cols[i % 3].number_input(f"{i+1}号艇", value=6.50 + (i*0.02), step=0.01, format="%.2f", key=f"r_{i}") for i in range(6)]
    st.write("【独自補正値】")
    h_cols = st.columns(3)
    offs = [h_cols[i % 3].number_input(f"{i+1}補正", value=(-0.03 if i == 0 else 0.00), step=0.01, format="%.2f", key=f"o_{i}") for i in range(6)]
...
def get_t_style(t):
    if t == best_t: return "background:#ef4444 !important; color:white !important; border:2px solid #333 !important;"
    if t == second_t: return "background:#fef08a !important; color:#854d0e !important; border:2px solid #333 !important;"
    return "background:white !important; color:#333 !important;"

# --- 3. CSSスタイル定義（分離して安全に読み込み） ---
# これによりコード漏れを防ぎ、image.pngのような質感を再現します
st.markdown("""
<style>
