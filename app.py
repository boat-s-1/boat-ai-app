...
with st.sidebar.expander("🤝 最下部：3人の合議", expanded=True):
    ...
    total_text = st.text_area("合議の詳細", value="3人の見解が一致。1号艇の逃げは揺るぎません！")
...
def get_style(t):
    if t == best_t: return "background:#ef4444; color:white; border:2px solid #333;" # 1位（赤）
    if t == second_t: return "background:#fef08a; color:#854d0e; border:2px solid #333;" # 2位（黄）
    return "background:white; color:#333;"

# --- 3. CSSスタイル定義（ここが新聞のクオリティを決めます） ---
st.markdown("""
<style>
    ...
    .data-table th { background: #edf2f7; border: 1px solid #333; padding: 6px; font-size: 12px; }
    ...
    .meter-needle { position: absolute; bottom: 0; left: 50%; width: 3px; height: 50px; background: #1a1a1a; transform-origin: bottom center; }
...
final_tds = "".join([f'<td style="{get_style(t)}">{t:.2f}</td>' for t in final_times])
...
<div class="newspaper-base">
    ...
    <div class="news-grid">
        ...
        <div style="flex: 1.3;">
            <div class="news-box" style="border-color: #f59e0b; background: #fffcf5;">
                ...
                <div style="display: flex; align-items: center; justify-content: space-around; margin: 10px 0;">
                    <div class="meter-box">
                        ...
                        <div class="meter-needle" style="transform: translateX(-50%) rotate({needle_deg}deg);"></div>
                    </div>
                    <div style="text-align: center; font-size: 28px; font-weight: bold; color: #ef4444;">{i_exp}%</div>
                </div>
            ...
            <div class="news-box" style="border-color: #3b82f6; background: #f0f9ff;">
                ...
                <div style="background: #1e293b; color: white; padding: 3px; text-align: center; border-radius: 4px; font-size: 14px; font-weight: bold; margin-bottom: 5px;">{h_eval}</div>
            ...
            <div class="news-box" style="border-color: #eab308; background: #fffdf0;">
                ...
                <div style="text-align: center; background: #ef4444; color: white; padding: 3px; border-radius: 4px; font-weight: bold; font-size: 13px;">判定：{k_eval}</div>
    ...
    <div style="text-align: center; margin-top: 15px; font-size: 10px; color: #94a3b8;">(c) BOAT STRIKE - Birthstones新聞係</div>
</div>
"""
