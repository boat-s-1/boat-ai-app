...
# --- 2. ロジック計算 ---
final_t = [round(raw_t[i] + offsets[i], 2) for i, offsets in enumerate(offs)]
sorted_unique = sorted(list(set(final_t)))
best_t = sorted_unique[0]
second_t = sorted_unique[1] if len(sorted_unique) > 1 else None

def get_t_style(t):
    if t == best_t: return "background:#ef4444; color:white; border:1px solid #333;"
    if t == second_t: return "background:#fef08a; color:#854d0e; border:1px solid #333;"
...
<style>
    ...
    /* 共通ボックス */
    .news-box { border: 2px solid #333; border-radius: 12px; padding: 15px; background: #fff; margin-bottom: 15px; min-height: 100px; }
...
# 背景となる「紙の枠」を開始
st.markdown('<div style="background-color: white; width: 920px; margin: 0 auto; padding: 40px; border: 2px solid #333; box-shadow: 12px 12px 0px #cbd5e0;">', unsafe_allow_html=True)
...
st.markdown(f"""
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 4px double #333; padding-bottom: 15px; margin-bottom: 25px;">
        ...
        <div style="text-align: right; font-size: 12px;">{formatted_date} 第23号</div>
    </div>
""", unsafe_allow_html=True)

# メインコンテンツ (左 1.7 : 右 1.3)
...
with col_right:
    ...
    # 右側：キイナ
    st.markdown(f"""
        <div class="news-box" style="border-color: #eab308; background: #fffdf0; margin-bottom:0;">
